...
class DBManagerTest {
    ...
    void singleInsertIntoDB() throws Exception {
        String insertQuery = "INSERT INTO chunk_owners(file_hash, chunk_hash, owner_ip, owner_port, is_seeder, is_active) " +
                "VALUES('file-hash-1', 'chunk-hash-1', 'localhost', '12345', '1', '1');";
        testDB.singleUpdate(insertQuery);

        String selectQuery = "SELECT file_hash, chunk_hash, owner_ip, owner_port, is_active FROM chunk_owners " +
        ...
        resultString = "[{\"file_hash\":\"" + fileHash + "\",\"chunk_hash\":\"" + chunkHash +  "\"," +
                "\"owner_ip\":\"" + ownerIP + "\",\"owner_port\":\"" + ownerPort + "\",\"is_active\":\"" + isActive + "\"}]";

        expectedString = "[{\"file_hash\":\"file-hash-1\",\"chunk_hash\":\"chunk-hash-1\"," +
                "\"owner_ip\":\"localhost\",\"owner_port\":\"12345\",\"is_active\":\"t\"}]";
